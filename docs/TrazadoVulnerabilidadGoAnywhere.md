# Apartado 1 - Trazado de vulnerabilidad

En este apartado muestro el desarrollo de la [Actividad- TrazadoVulnerabilidad](https://github.com/jmmedinac03vjp/PuestaProduccionSegura/blob/main/Unidad2-NivelesSeguridad/Actividad-TrazadoVulnerabilidad/README.md).

El trazado de la vulnerabilidad de la que se informa se corresponde a la de [este artículo](https://www.incibe.es/empresas/avisos/vulnerabilidad-critica-de-omision-de-autenticacion-en-goanywhere-mft-de-fortra).

---

En primer lugar, me dirijo al artículo del [INCIBE](https://www.incibe.es/).
...
También encontramos una descripción que nos dice que la vulnerabilidad permite a los ciberdelincuentes crear un usuario administrador a partir del portal de administración, eludiendo los requisitos de autenticación en el acceso a un servicio.
...
Se muestra la solución, que sería actualizar a la versión 7.4.1 o superior o eliminar el archivo _InitialAccountSetup.xhtml_ en el directorio de instalación en caso de despliegues sin contenedor. Con despliegues en contenedor, habría que reemplazar dicho archivo por uno limpio y reiniciar.

Se dan enlaces para obtener más información desde el portal de clientes de GoAnywhere.

![Artículo INCIBE](img/Trazado/articulo_2.png)

---

Llegados a este punto, sabemos a qué elementos afecta la vulnerabilidad, pero no tenemos enlace para dirigirnos a las diferentes listas, por lo que me dirigiré al enlace del apartado de referencias del artículo: [!enlace referencia](https://www.fortra.com/security/advisories/product-security/fi-2024-001).

En la referencia, se nos vuelve a describir que la vulnerabilidad se debe a un Bypass que pueden realizar los ciberdelicuentes desde el portal de administración. También se nos indica la severidad, el impacto que tiene y las soluciones. Básicamente, lo mismo que teníamos en el artículo del INCIBE.

Lo que nos encontramos aquí es la identificación de una única vulnerabilidad, ya que se nos indica un único [!CVE](https://www.incibe.es/incibe-cert/alerta-temprana/vulnerabilidades). CVE es una lista mantenida por el ![MITRE](https://attack.mitre.org/) en donde, cuando alguien encuentra una vulnerabilidad envía un reporte y ellos la estudian para determinar si ya está reportada o no, y en tal caso, asignarle un número identificativo. Este número consistiría en CVE, seguido del número de año y un número correlativo.
...
En ambos sitios, la información obtenida es muy similar.

## NVD

Encontramos la descripción de la vulnerabilidad y la métrica. La métrica es la forma de calcular el nivel de criticidad de la vulnerabilidad (CVSS).

Si nos fijamos en el CVSS, depende de varios factores:

* Vector de ataque: La facilidad del vector de ataque depende. No sería lo mismo necesitar acceso físico al ordenador para realizar el ataque que realizarlo desde cualquier sitio, a través de la red. En este caso se realiza desde la red.
* Complejidad del ataque: Si es sencillo realizarlo o no. En este caso es bajo.
* Privilegios: La cantidad de privilegios que se requieren para hacerlo. No sería lo mismo necesitar ser un usuario logeado para hacerlo que hacerlo desde "fuera". En este caso no se necesitan privilegios.
...
* Confidencialidad, integridad y disponibilidad: Afecta a las tres de manera muy alta.
* Conocimiento: Si es conocida tiene más impacto que si no lo es. En este caso tiene un 5.9 de impacto.
...
En el MITTRE encontramos información muy similar a la de la NVD, pero con menos detalles. Por ejemplo, en la métrica, no se detallan los factores para calcular la criticidad de la vulnerabilidad.
...
2. Hacer uso de marcos basados en MVC como Struts.
